...
function setTitlesPage(page) {
  ...
  $.getJSON("/titles?size=5&page=" + page, function(data) {
    var tbl_body = "";
    var odd_even = false;
    $.each(data._embedded.titles, function(index, value) {
      var tbl_row = "<td align=\"center\"><a class=\"btn btn-default\" onclick=\"editTitle('" + encodeURI(value._links.self.href)
          + "')\" data-toggle=\"modal\" data-target=\"#myModal\"><em class=\"glyphicon glyphicon-pencil\" onclick=\"setModal('/title-edit.html', 'Title anlegen', '/articles/1')\"></em></a>";
      ...
      tbl_row += "<td class=\"hidden\" onclick=\"setModal('/title-edit.html', 'Titel anlegen', '/titles')\">" + value._links.self.href + "</td>";
      ...
      tbl_row += "<td role=\"button\" data-toggle=\"modal\" data-target=\"#myModal\" onclick=\"showAddArtistModal('/add-artist.html', '" + value.label + "', '" + value._links.artist.href + "')\">" + artist + "</td>";
