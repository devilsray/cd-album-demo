...
function setArtistsPage(page) {
  ...
  $.getJSON("/artists?size=5&page=" + page, function(data) {
    ...
    $.each(data._embedded.artists, function(index, value) {
      ...
      tbl_row += "<a class=\"btn btn-default\" onclick=\"setArtistToTitle('" + encodeURI(value._links.self.href) + "')\" data-toggle=\"modal\" data-target=\"#myModal\">";
      tbl_row += "<em class=\"glyphicon glyphicon-plus-sign\" onclick=\"setModal('/artist-edit.html', 'Künstler anlegen', '/articles/1')\"></em>";
      tbl_row += "</a>";
      tbl_row += "</td>";
