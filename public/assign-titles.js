function setTitlesPage(page) {
  ...
  $.getJSON("/titles?size=5&page=" + page, function(data) {
    ...
    $.each(data._embedded.titles, function(index, value) {
      ...
      var tbl_row = "<td id=\"title-" + (knownIds.length - 1) + "\" align=\"center\"><a class=\"btn btn-default\" onclick=\"addTitleToTarget('" + encodeURI(value._links.self.href)
          + "')\" data-toggle=\"modal\" data-target=\"#myModal\"><em class=\"glyphicon glyphicon-plus-sign\" onclick=\"setModal('/artist-edit.html', 'Künstler anlegen', '/articles/1')\"></em></a>";
      tbl_row += "</td>";
      ...
      tbl_row += "<td>" + artist + "</td>";
      tbl_body += "<tr data-id=\"" + value._links.self.href + "\">" + tbl_row + "</tr>";
    });
    ...
    $.getJSON($(".target-uri").val(), function(data) {
      var tbl_body = "";
      var odd_even = false;
      $.each(data._embedded.titles, function(index, value) {
